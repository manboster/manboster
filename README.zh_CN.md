...
虽然现在只有telegram接入（后续版本会加入飞书的接入）和openai兼容api的大语言模型接入，但是我们仍然在给项目添加新功能！
...
3. 哈吉米，一个大部分情况下在本地运行的小模型，它可以在你需要的时候帮助你托管工具调用请求。 它觉得不安全的时候才会问你的意见，让你拍板。
4. 零信任架构设计，守门员系统会拦截住所有的工具调用请求并逐一放行，有缓存期而不是永久的允许。
5. 可插拔可配置的内置工具，你完全可以用 `manboster config` 启用或禁用一个工具。
6. 一个内置的搜索工具，它内置了基于 `go-rod` 的无头浏览器，你可以用它做搜索，当然，你也可以用自己的搜索API Key来搜索。 当然，你也可以用自己的搜索API Key来搜索
...
14. [计划中功能] MamboHub，一个让你可以轻松下载并使用技能与插件的分发中心。我们当然也会支持类似于 ClawHub 的 skills 安装 我们当然也会支持类似于 ClawHub 的 skills 安装。
15. [计划中功能] 你可以使用 .manboplugin 文件安装插件，或者是使用 manbodev 辅助工具来开发和编写自己的插件！

## 快速上手

1. 下载适合你的可执行二进制文件并通过终端双击或来打开它。
2. 如果你是第一次运行的话，你需要配置你的 Manboster，配置就可以了。
3. 享受你的虾头小助手吧！

如果你下载了 Go 语言的开发环境的话，你只需要在终端输入以下命令就可以把 manboster 安装到你的电脑了： go install github.com/manboster/manboster/cmd/manboster@latest

更多详见 [快速开始文档](https://manboster.dev/zh-cn/docs/quickstart.html)

直接启动不会开启守护进程，输入 `manboster start` 启动守护进程。
...
文档里面没有给出解决方案或者是解决方案无法解决问题时再去issue搜索这个问题

请先在互联网上搜索相关的问题并寻找解决方案， 如果还没有，请开issue。在开issue之前，请先充分阅读并理解 [提问的智慧](https://github.com/ryanhanwu/How-To-Ask-Questions-The-Smart-Way/blob/main/README-zh_CN.md) 以给你我减少不必要的时间，冲突和麻烦。
