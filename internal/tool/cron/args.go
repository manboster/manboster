...
type RunArgs struct {
	...
	MessageType MessageType `json:"message_type" description:"The message's type. Only valid when name = 'set'. It would be enum, only 2 values: 'text' and 'prompt', 'text' means the message would be sent as a text message, 'prompt' means the message would be user's prompt to activate the model." enum:"text,prompt" example:"text"`
	...
	Ignore      IgnoreType  `json:"ignore" description:"Only valid when 'name' = 'set'. It would be enum, only 2 values: 'ignore' or 'hachimi', ignore means tool calls would be automatically executed while hachimi means hachimi enabled in tool call process and notify user. Please use hachimi unless user wants to ignore.'" example:"hachimi" enum:"hachimi,ignore"`
}
