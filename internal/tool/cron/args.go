...
type RunArgs struct {
	...
	JobName     string      `json:"job_name" description:"The job's name, it's unique." example:"job_cronjob"`
	MessageType MessageType `json:"message_type" description:"The message's type. Only available when name = 'set'. It would be enum, only 2 values: 'text' and 'prompt', 'text' means the message would be sent as a text message, 'prompt' means the message would be user's prompt to activate the model." enum:"text,input" example:"text"`
