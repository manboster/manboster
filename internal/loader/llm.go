...
func LoadLLMProvider(ctx context.Context, llmConfig config.LLMConfig, provider llm.Provider) (llm.Provider, error) {
	...
	if err != nil {
		color.Red(fmt.Sprintf("[Manboster Engine] Activate %s LLM API Error! Message: %s", provider.DisplayName()))
	} else {
		color.Green(fmt.Sprintf("[Manboster Engine] Activate LLM Provider %q successful!", llmConfig.Provider))
	}
...
func LoadLLMProviders(ctx context.Context, llmConf []config.LLMConfig) ([]llm.Provider, error) {
	...
	for _, llmConfig := range llmConf {
		...
		if err != nil {
			color.Red(fmt.Sprintf("[Manboster Engine] There is no provider named %q when importing llm providers. Please check your configuration.", llmConfig.Provider))
			return nil, err
		}
