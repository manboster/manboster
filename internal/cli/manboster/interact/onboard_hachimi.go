...
func runOnboardHachimiConfigs(p cli.Provider) (config.HachimiConfigs, error) {
	...
	confirm, err := p.Prompt(i18n.T(keys.OnboardHachimiFeaturePrompt), i18n.T(keys.OnboardHachimiEnableQuestion), i18n.T(keys.UIBtnYes), i18n.T(keys.UIBtnNo))
	if err != nil {
		return conf, err
	}
	...
	for {
		...
		if err != nil {
			err := p.Alert(i18n.T(keys.CliWizardTitle), i18n.Te(keys.OnboardHachimiConfigError, "", err))
			if err != nil {
				return conf, err
			}
			continue
		...
		ok, err := p.Prompt(i18n.T(keys.OnboardHachimiAddedCount, len(hachimiConfigs)), "", i18n.T(keys.UIBtnContinue), i18n.T(keys.UIBtnExit))
		if err != nil {
			return conf, err
		}
