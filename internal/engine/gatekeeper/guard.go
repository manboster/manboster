...
func (s *Service) Guard(ctx context.Context, instance chat.Provider, msg *chat.Message, toolProvider tool.Provider, req llm.MessageToolCallRequestPayload, sid string) (bool, error) {
	...
	m, mT := s.sessionManager.Ignorance.GetMark(overallUd)
	if m && mT == gatekeeper.MarkCancelAll {
		return false, fmt.Errorf("user manually canceled your all tool calls in next 10 minutes")
	}
	if m && (mT == gatekeeper.MarkHachimiAll || mT == gatekeeper.MarkHachimiAllSuspicious) {
		// run hachimi here...
		return s.HachimiHandler(ctx, instance, msg, toolProvider, req, ud, sid)
	}
	if m && mT == gatekeeper.MarkContinueAll {
		return true, nil
	}

	mark, markType := s.sessionManager.Ignorance.GetMark(ud)
	if (mark && markType == gatekeeper.MarkHachimi && requireUserType <= actualUserType) || msg.MessageType&chat.MessageFromCron != 0 {
		return s.HachimiHandler(ctx, instance, msg, toolProvider, req, ud, sid)
	}
	if mark && markType == gatekeeper.MarkIgnore && requireUserType <= actualUserType || msg.MessageType&chat.MessageFromCronIgnore != 0 {
		return true, nil
	}
	...
	return s.Select(ctx, instance, msg, selection, util.DescribeToHuman(req, toolProvider), func(msg *chat.Message) (bool, error) {
		...
		// get by session id
		m, mT := s.sessionManager.Ignorance.GetMark(overallId)
		if m && mT == gatekeeper.MarkCancelAll {
			return false, fmt.Errorf("user manually canceled your all tool calls in next 10 minutes")
		}
		if m && (mT == gatekeeper.MarkHachimiAll || mT == gatekeeper.MarkHachimiAllSuspicious) {
		...
		mark, markType := s.sessionManager.Ignorance.GetMark(id)
		if mark && markType == gatekeeper.MarkHachimi {
			// run hachimi here...
			return s.HachimiHandler(ctx, instance, msg, toolProvider, req, id, sid)
		}
		if mark && markType == gatekeeper.MarkIgnore {
		...
		case guardSelectHachimi:
			s.sessionManager.Ignorance.SetMark(id, true, ttl, gatekeeper.MarkHachimi)
			return true, errors.New(i18n.T(keys.GatekeeperHachimiActivated, map[string]any{
				"Next": ttl / 60,
			}))
		case guardSelectHachimiAll:
			s.sessionManager.Ignorance.SetMark(overallId, true, 60*60, gatekeeper.MarkHachimiAll)
			return true, errors.New(i18n.T(keys.GatekeeperHachimiActivated, map[string]any{
				"Next": 60,
			}))
		case guardSelectIgnore:
			s.sessionManager.Ignorance.SetMark(id, true, ttl, gatekeeper.MarkIgnore)
		...
		case guardSelectContinueAll:
			s.sessionManager.Ignorance.SetMark(overallId, true, 10*60, gatekeeper.MarkContinueAll)
		...
		case guardSelectCancelIgnore:
			s.sessionManager.Ignorance.SetMark(id, true, 15*60, gatekeeper.MarkCancel)
		...
		case guardSelectCancelAll:
			...
			s.sessionManager.Ignorance.SetMark(overallId, true, 10*60, gatekeeper.MarkCancelAll)
